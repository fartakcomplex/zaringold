...
const DB_PATH = (() => {
  // Read DATABASE_URL from .env to determine DB path
  try {
    ...
    if (match) {
      ...
      return path.join(PROJECT_ROOT, dbUrl);
    }
  } catch {}
...
async function exportDatabase(): Promise<string | null> {
  if (!fileExists(DB_PATH)) {
    console.log('[easy-installer] Database not found, skipping export');
    return null;
  }

  try {
    // Use sqlite3 command-line tool to dump the database
    ...
    console.log(`[easy-installer] Database exported: ${formatBytes(sqlDump.length)}`);
    return dumpPath;
  } catch (err) {
    console.error('[easy-installer] Database export failed:', err);
    // Try alternative method using better-sqlite3 or Prisma
    try {
      ...
      let sql = '';
      sql += '-- Mili Gold Database Export\n';
      sql += `-- Generated: ${new Date().toISOString()}\n`;
      sql += '-- Do NOT edit this file manually\n\n';
      ...
      for (const table of tables) {
        ...
        const createStmt = await prisma.$queryRaw<Array<{ sql: string }>>(
