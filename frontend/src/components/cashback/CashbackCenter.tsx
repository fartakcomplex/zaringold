...
import {Gift, Coins, Gem, Clock, CheckCircle2, Loader2, AlertCircle, ChevronDown, ChevronUp, PartyPopper, Sparkles, Trophy, Wallet} from 'lucide-react';
...
function AnimatedCoinIcon({ className = 'size-5' }: { className?: string }) {
  return (
    <motion.div variants={coinSpinVariants} animate="animate" style={{ perspective: 200 }}>
      <Coins className={cn('text-amber-500', className)} />
    </motion.div>
  );
}

function AnimatedGoldIcon({ className = 'size-5' }: { className?: string }) {
  return (
    <motion.div variants={floatVariants} animate="animate">
      <Gem className={cn('text-gold', className)} />
    </motion.div>
  );
}
      ...
      if (resData.success && resData.cashback) {
        ...
        setData({
          ...
          rewards: [
            {
              id: '1',
              title: 'کش‌بک خرید هفتگی',
              value: 350000,
              type: 'toman',
              status: 'available',
              expiryDate: new Date(Date.now() + 7 * 86400000).toISOString(),
              description: 'کش‌بک خرید طلا در هفته گذشته',
            },
            {
              id: '2',
              title: 'جایزه دعوت دوست',
              value: 50,
              type: 'gold_mg',
              status: 'available',
              expiryDate: new Date(Date.now() + 30 * 86400000).toISOString(),
              description: '۵۰ میلی‌گرم طلای هدیه برای دعوت موفق',
            },
            {
              id: '3',
              title: 'کش‌بک وام طلایی',
              value: 180000,
              type: 'toman',
              status: 'available',
              expiryDate: new Date(Date.now() + 14 * 86400000).toISOString(),
              description: 'کش‌بک تسویه اقساط وام',
            },
            {
              id: '4',
              title: 'کش‌بک خرید ماهانه',
              value: 520000,
              type: 'toman',
              status: 'claimed',
              claimedAt: new Date(Date.now() - 10 * 86400000).toISOString(),
            },
            {
              id: '5',
              title: 'جایزه ثبت‌نام',
              ...
              type: 'gold_mg',
              status: 'claimed',
              claimedAt: new Date(Date.now() - 30 * 86400000).toISOString(),
            },
            {
              id: '6',
              title: 'کش‌بک منقضی شده',
              value: 250000,
              type: 'toman',
              status: 'expired',
        ...
        if (data) {
          setData({
            ...
            rewards: data.rewards.map((r) =>
              r.id === rewardId ? { ...r, status: 'claimed' as const, claimedAt: new Date().toISOString() } : r
            ),
          });
        }
      } else {
    ...
    return (
      <Card className="border-red-200 bg-red-50/50 dark:border-red-900/50 dark:bg-red-950/20">
        <CardContent className="flex flex-col items-center gap-2 py-10">
          ...
          <button onClick={fetchCashback} className="text-xs text-gold hover:underline">
            تلاش مجدد
          </button>
        </CardContent>
      </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-2xl font-bold gold-gradient-text tabular-nums">
                  {formatToman(data.totalEarned)}
                </p>
                <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    ...
                    {claimedRewards.length > 0 && (
                      <span className="ms-2 text-muted-foreground/60">| {claimedRewards.length} دریافت شده</span>
                    )}
                  </span>
                </div>
              </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className={cn(
                        ...
                        reward.type === 'toman' ? 'bg-amber-500/10' : 'bg-gold/10'
                      )}>
                        {reward.type === 'toman' ? (
                          <AnimatedCoinIcon />
                        ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className={cn('text-[10px] shrink-0', statusConfig[reward.status].className)}>
                            <StatusIcon className="size-3 ms-1" />
                            فعال
                          </Badge>
                        </div>
                        <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
                          <span className="font-bold text-foreground">
                            {reward.type === 'toman'
                              ...
                              : `${formatNumber(reward.value)} میلی‌گرم طلا`}
                          </span>
                          {reward.expiryDate && (
                            <span className="flex items-center gap-1">
                              <Clock className="size-3" />
                              تا {formatDate(reward.expiryDate)}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <Button
                        size="sm"
                        className={cn(
                          'shrink-0 bg-gradient-to-r from-gold to-amber-500 text-gold-dark hover:from-gold/90 hover:to-amber-500/90 btn-gold-shine',
                          claimingId === reward.id && 'opacity-70 pointer-events-none'
                        )}
                    ...
                    {reward.description && (
                      <p className="mt-2 text-xs text-muted-foreground/70 ms-[52px]">{reward.description}</p>
                    )}
                  </CardContent>
                </Card>
              </motion.div>
            ...
            <CardContent className="flex flex-col items-center gap-3 py-10">
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground/70">
                  با خرید و فروش طلا، کش‌بک کسب کنید
                </p>
              </div>
            </CardContent>
          </Card>
                ...
                <CardContent className="flex items-center justify-between p-4 hover:bg-muted/30 transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className="text-[10px]">
                      {claimedRewards.length + expiredRewards.length}
                    </Badge>
                  </div>
                  {historyOpen ? (
                        ...
                        <div key={reward.id} className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className={cn(
                              ...
                              reward.type === 'toman' ? 'bg-amber-500/10' : 'bg-gold/10',
                              reward.status === 'expired' && 'opacity-50'
                            )}>
                              {reward.type === 'toman' ? (
                            ...
                            <div className="min-w-0 flex-1">
                              <p className={cn('text-sm font-medium truncate', reward.status === 'expired' && 'line-through text-muted-foreground')}>
                                {reward.title}
                              </p>
                              <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                                <span>
                                  {reward.type === 'toman'
                                    ? formatToman(reward.value)
                                    : `${formatNumber(reward.value)} میلی‌گرم`}
                                </span>
                                {reward.claimedAt && (
                                  <span>دریافت: {formatDate(reward.claimedAt)}</span>
                                )}
                              </div>
                            </div>
                            <Badge variant="outline" className={cn('text-[10px] shrink-0', status.className)}>
                              <HistoryIcon className="size-3 ms-1" />
                              {status.label}
                            </Badge>
                          </div>
                        </div>
