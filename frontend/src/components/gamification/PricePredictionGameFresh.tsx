...
export default function PricePredictionGameFresh() {
  ...
  const handleSubmitPrediction = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPredictions((prev) => [
          { id: data.prediction?.id || 'new', predictedPrice, targetDate: getTomorrow(), createdAt: new Date().toISOString(), status: 'pending' },
          ...prev,
        ]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-24">
      ...
      <div className="flex flex-col items-center gap-3 py-6 text-center">
        <div className="flex size-20 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-500/20 to-orange-500/5 border-2 border-rose-500/30">
          <Crosshair className="size-10 text-rose-500" />
        </div>
        <div>
          <h2 className="text-2xl font-extrabold text-rose-500">پیش‌بینی قیمت طلا</h2>
          <p className="mt-1 text-sm text-muted-foreground">قیمت فردا را حدس بزنید و امتیاز کسب کنید</p>
        </div>
      </div>

      {/* ── Current Price ── */}
      <Card className="overflow-hidden border-rose-300/30" id="pg-rules">
      ...
      {/* ── Stats Row ── */}
      {stats && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4" id="pg-stats">
          {[
      ...
      <div id="pg-predict">
        <Card className="overflow-hidden">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <CardContent className="space-y-5 pt-4">
                {hasTodayPrediction ? (
                  ...
                  <>
                    ...
                    <Button className="w-full bg-gradient-to-l from-rose-500 to-orange-500 text-white hover:from-rose-600 hover:to-orange-600" onClick={handleSubmitPrediction} disabled={submitting || !user?.id}>
              ...
              <CardContent className="pt-4">
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {predictions.map((pred) => (
                    <div key={pred.id} className="flex items-center gap-3 rounded-xl border border-border/50 p-3 hover:bg-muted/30 transition-colors">
                      <div className={cn(
                        ...
                        {pred.status === 'correct' ? <CheckCircle className="size-4 text-emerald-500" /> :
                         pred.status === 'incorrect' ? <XCircle className="size-4 text-red-500" /> :
            ...
            {/* ── Leaderboard Tab ── */}
            <TabsContent value="leaderboard" id="pg-leaderboard">
              <CardContent className="pt-4">
                <div className="space-y-2 max-h-96 overflow-y-auto">
